...
import {
  ...
  gameState,
  getPawnAtBoardIndex,
  isOccupiedByOwnPawnBoard,
  isOccupiedByOwnPawnSafe,
  sendPawnToStart,
...
export function debugSafetyEntries() {
  ...
  PLAYER_START_INFO.forEach((info, idx) => {
    console.log(
      `Player ${idx} (${PLAYERS[idx].name}): Safety Entry at board index ${info.safetyEntryIndex}`
    );
    if (BOARD_PATH[info.safetyEntryIndex]) {
      console.log(
        `  - Grid position: (${BOARD_PATH[info.safetyEntryIndex].gridX}, ${
          BOARD_PATH[info.safetyEntryIndex].gridY
        })`
      );
      console.log(
        `  - Pixel coordinates: (${BOARD_PATH[info.safetyEntryIndex].pixelX}, ${
          BOARD_PATH[info.safetyEntryIndex].pixelY
        })`
      );
      ...
      if (idx === 2) {
        console.log(
          `  - YELLOW CHECK: Position 30 is at (${BOARD_PATH[30].gridX}, ${BOARD_PATH[30].gridY})`
        );
        console.log(
          `  - YELLOW CHECK: Position 30 pixel coords: (${BOARD_PATH[30].pixelX}, ${BOARD_PATH[30].pixelY})`
        );
        ...
        const gridDist =
          Math.abs(pos30.gridX - pos31.gridX) +
          Math.abs(pos30.gridY - pos31.gridY);
        console.log(
          `  - YELLOW CHECK: Grid distance between pos 30 and 31: ${gridDist}`
        );

        // Check distance to safety zone start
        const safetyStart = SAFETY_ZONES[2][0];
        const dist30ToSafety =
          Math.abs(pos30.gridX - safetyStart.gridX) +
          Math.abs(pos30.gridY - safetyStart.gridY);
        const dist31ToSafety =
          Math.abs(pos31.gridX - safetyStart.gridX) +
          Math.abs(pos31.gridY - safetyStart.gridY);
        console.log(
          `  - YELLOW CHECK: Distance from pos 30 to safety start: ${dist30ToSafety}`
        );
        console.log(
          `  - YELLOW CHECK: Distance from pos 31 to safety start: ${dist31ToSafety}`
        );
      }
    } else {
...
export function diagnoseSafetyZones() {
  ...
  SAFETY_ZONES.forEach((zone, playerIdx) => {
    ...
    if (zone.length !== SAFETY_ZONE_LENGTH) {
      console.error(
        `  - ERROR: Expected ${SAFETY_ZONE_LENGTH} spaces, found ${zone.length}`
      );
    }

    zone.forEach((space, idx) => {
      console.log(
        `  - Space ${idx}: Grid(${space.gridX},${space.gridY}), Pixel(${space.pixelX},${space.pixelY})`
      );
    ...
    for (let safePos = 0; safePos < SAFETY_ZONE_LENGTH; safePos++) {
      ...
      for (let card = 1; card <= 6; card++) {
        ...
        if (targetPos < SAFETY_ZONE_LENGTH) {
          ...
          console.log(
            `      ✗ Invalid: Overshoots Home by ${
              targetPos - SAFETY_ZONE_LENGTH
            } spaces`
          );
        }
        ...
        console.log(
          `      → Got ${moves.length} moves from getPossibleMovesForPawn()`
        );
        ...
        if (moves.length === 0) {
          ...
          const move = moves[0];
          console.log(
            `      → Move: Type=${move.positionType}, Index=${move.positionIndex}, Coords=(${move.pixelX},${move.pixelY})`
          );
        }
      }
    }
...
function getMovesWithinSafetyZone(pawn, steps) {
  ...
  console.log(
    `Pawn ${pawn.id} (Player ${playerIndex}) at safety position ${currentSafePos}, steps=${steps}`
  );
  ...
  if (targetSafePos < SAFETY_ZONE_LENGTH) {
    ...
    if (!occupied) {
      console.log(
        `Valid move within safety zone from ${currentSafePos} to ${targetSafePos}`
      );
      moves.push({
        type: "move",
        positionType: "safe",
  ...
  // Case 3: Overshoots Home
    ...
    // Overshot home
    console.log(
      `Overshot home from safety position ${currentSafePos} with ${steps} steps`
    );
  }
...
export function getPossibleMovesForPawn(pawn, card, stepsOverride = null) {
  ...
  if (isPawnInSafety(pawn)) {
    ...
    if (!isNaN(numericCardValue) && numericCardValue > 0 && cardValue !== "4") {
      ...
      moves.push(
        ...safetyzoneMovementEngine(pawn, numericCardValue, forceAllowMove)
      );
    }
    ...
    if (cardValue === "7") {
      ...
      for (let steps = 1; steps <= 7; steps++) {
        const safetyMoves = safetyzoneMovementEngine(
          pawn,
          steps,
          forceAllowMove
        );
  ...
  if (cardValue === "4") {
    ...
    if (isPawnOnBoard(pawn)) {
      ...
      if (fwd10.type !== "invalid") {
        console.log(` -> FWD 10 move is valid:`, fwd10);
        moves.push({ type: "move", ...fwd10 });
      } else {
        console.log(` -> FWD 10 move is invalid`);
      }

      // Calculate Backward 1
      console.log(
        `Checking BWD 1 for Pawn ${pawn.id} at ${pawn.positionIndex}`
      );
      const targetIndexBwd =
        (pawn.positionIndex - 1 + PATH_LENGTH) % PATH_LENGTH;
      console.log(` -> Target index for BWD 1: ${targetIndexBwd}`);
      if (!isOccupiedByOwnPawnBoard(targetIndexBwd, pawn.playerIndex)) {
        console.log(` -> BWD 1 move to ${targetIndexBwd} is valid`);
        moves.push({
          type: "move",
          positionType: "board",
          positionIndex: targetIndexBwd,
          pixelX: BOARD_PATH[targetIndexBwd].pixelX,
          pixelY: BOARD_PATH[targetIndexBwd].pixelY,
        });
      } else {
        console.log(
          ` -> BWD 1 move to ${targetIndexBwd} is invalid (occupied by own)`
        );
      }
    }
...
export function calculateForwardSteps(pawn, steps, startInfo) {
  ...
  console.log(
    `Calculating forward steps: Pawn at ${currentType} position ${currentPos}, moving ${steps} steps`
  );
  ...
  if (currentType === "safe") {
    ...
    if (targetSafeIndex < SAFETY_ZONE_LENGTH) {
      ...
      if (!isOccupiedByOwnPawnSafe(pawn.playerIndex, targetSafeIndex)) {
        ...
        console.log(
          `Direct movement within safety zone from ${currentPos} to ${targetSafeIndex}`
        );
        return {
          positionType: "safe",
          positionIndex: targetSafeIndex,
  ...
  if (currentType === "board") {
    ...
    for (let i = 1; i <= steps; i++) {
      ...
      if (checkPos === safetyEntry && i < steps) {
        console.log(
          `Invalid move: Cannot pass through safety entry point at ${safetyEntry}`
        );
        return { type: "invalid" };
      }
    }
  ...
  while (stepsLeft > 0) {
    ...
    if (currentType === "board") {
      ...
      if (currentPos === startInfo.safetyEntryIndex && stepsLeft === 1) {
        console.log(
          `ENTRY CHECK: Validating move to safety entry point ${currentPos} for player ${pawn.playerIndex}`
        );
        ...
        for (const player of gameState.players) {
          if (player.index === pawn.playerIndex) {
            ...
            for (const otherPawn of player.pawns) {
              console.log(
                `ENTRY CHECK: Pawn ${otherPawn.id} is at ${otherPawn.positionType} position ${otherPawn.positionIndex}`
              );
              ...
              if (
                ...
                console.log(
                  `Invalid move: Entry point ${currentPos} already occupied by own pawn`
                );
                entryOccupied = true;
                break;
              }
        ...
        console.log(
          `Landed on safety entry point at ${currentPos}, marking as 'entry'`
        );
        console.log(
          `SAFETY ENTRY DEBUG: Player ${pawn.playerIndex} (${
            PLAYERS[pawn.playerIndex].name
          })`
        );
        console.log(
          `SAFETY ENTRY DEBUG: Entry point defined as ${startInfo.safetyEntryIndex}`
        );
      ...
      if (currentPos === SAFETY_ZONE_LENGTH) {
        ...
        if (stepsLeft === 1) {
          console.log(
            `Exact move to home from safety position ${currentPos - 1}`
          );
          return {
            positionType: "home",
            positionIndex: -1,
          ...
          console.log(
            `Overshot home from safety position ${currentPos - 1} with ${
              stepsLeft - 1
            } steps remaining`
          );
          return { type: "invalid" };
  ...
  if (currentType === "board") {
    ...
    const isAtSafetyEntry =
      startInfo && currentPos === startInfo.safetyEntryIndex;
...
export function executeSorry(startPawn, targetPawn) {
  ...
  gameState.message = `Sorry! Bumped ${
    PLAYERS[targetPawn.playerIndex].name
  }'s pawn!`;
...
export function executeSwap(playerPawn, opponentPawn) {
  ...
  gameState.message = `Swapped places with ${
    PLAYERS[opponentPawn.playerIndex].name
  }!`;
  ...
  if (slide && slide.colorIndex !== playerPawn.playerIndex) {
    ...
    if (
      bumpedPawn &&
      bumpedPawn !== opponentPawn &&
      bumpedPawn.playerIndex !== playerPawn.playerIndex
    ) {
      sendPawnToStart(bumpedPawn);
...
export function safetyzoneMovementEngine(pawn, steps, forceAllowMove = false) {
  ...
  if (
    !SAFETY_ZONES[playerIndex] ||
    SAFETY_ZONES[playerIndex].length !== SAFETY_ZONE_LENGTH
  ) {
    console.error(
      `ERROR: Safety zone for player ${playerIndex} is not properly configured`
    );
    return [];
  }
  ...
  if (targetPos < SAFETY_ZONE_LENGTH) {
    console.log(
      `[SAFETY ENGINE] Target is within zone at position ${targetPos}`
    );
    ...
    if (!occupied || forceAllowMove) {
      ...
      if (occupied && forceAllowMove) {
        console.log(
          `[SAFETY ENGINE] Position ${targetPos} is occupied but move forced!`
        );
      } else {
        console.log(
          `[SAFETY ENGINE] Creating move to safety position ${targetPos}`
        );
      }
      ...
      if (!targetSpace || !targetSpace.pixelX || !targetSpace.pixelY) {
        console.error(
          `ERROR: Missing coordinates for safety position ${targetPos}`
        );
        return [];
      }
      ...
      console.log(
        `[SAFETY ENGINE] Position ${targetPos} is occupied by own pawn. Move invalid.`
      );
    }
  }
  // CASE 2: Exact move to Home
    ...
    if (!homeCoord || !homeCoord.x || !homeCoord.y) {
      console.error(
        `ERROR: Missing home coordinates for player ${playerIndex}`
      );
      return [];
    }
  ...
  // CASE 3: Overshoots Home
    ...
    console.log(
      `[SAFETY ENGINE] Move invalid: overshoots Home by ${
        targetPos - SAFETY_ZONE_LENGTH
      } spaces`
    );
  }
